...
```env
APP_DOMAIN=bmrl.site
WEB_PORT=8080
PUBLIC_BASE_URL=https://bmrl.site
CORS_ORIGINS=https://bmrl.site
```
...
Массовое удаление гонщиков и гонок доступно только системному администратору (`ADMIN_LOGIN`) и требует отдельный bcrypt-пароль из `ADMIN_DANGER_PASSWORD_HASH`.

Перед ручным удалением проекта на Linux/Synology создай backup:

```bash
bash scripts/backup-project.sh
```

Сайт не имеет доступа к Docker-сокету, поэтому удаление самого проекта/хоста выполняется только вручную через SSH после проверки backup.
...
- сайт: `https://bmrl.site`
- healthcheck: `https://bmrl.site/health`
- API docs: `https://bmrl.site/api/docs`
