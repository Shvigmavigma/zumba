export const messages = {
  ru: {
    ...
    adminUsers: {
      ...
      systemHint: 'Значения хранятся в базе данных и применяются сразу: лимит считается отдельно для каждого пользователя, коэффициент общий для RER всех симуляторов, SR рассчитывается как значение за гонку × число завершённых гонок с учётом штрафов.',
  ...
  en: {
    ...
    adminUsers: {
      ...
      systemHint: 'Values are stored in the database and applied immediately: the limit is per user, the coefficient is shared by RER for all simulators, and SR is calculated as the per-race value × finished-race count with penalties applied.',
