...
const publicBracketColumns = computed(() => {
  ...
  for (const match of viewer.value?.matches || []) {
    ...
    if (!column) {
      const label = type === 'upper' && number === 1 ? '1 этап · До распределения' : type === 'upper' || type === 'lower' ? `${type === 'upper' ? 'Верхняя' : 'Нижняя'} · Раунд ${number}` : type === 'round' ? `Раунд ${number}` : competitionMatchStageLabel(match)
