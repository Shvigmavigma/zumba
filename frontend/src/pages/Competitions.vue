...
const publicBracketColumns = computed(() => {
  ...
  for (const match of viewer.value?.matches || []) {
    ...
    if (!column) {
      const label = type === 'upper' && number === 1 ? '1 этап · До распределения' : type === 'upper' || type === 'lower' ? `${type === 'upper' ? 'Верхняя' : 'Нижняя'} · Раунд ${number}` : type === 'semifinal' ? 'Полуфиналы' : type === 'round' ? `Раунд ${number}` : competitionMatchStageLabel(match)
      const depth = ['round', 'upper', 'lower'].includes(type) ? Math.max(0, number - 1) : 0
      column = {
        key,
        type,
        number,
...
.competition-public-bracket-column { row-gap:var(--competition-bracket-column-gap,10px); transform:translateY(var(--competition-bracket-column-offset,0px)); }
.competition-public-bracket-column.is-upper h3 { color:var(--primary-strong); }
.competition-public-bracket-column.is-lower h3 { color:var(--success); }
.competition-public-bracket-column.is-upper .competition-public-bracket-match { border-color:color-mix(in srgb,var(--accent) 45%,var(--border)); }
.competition-public-bracket-column.is-lower .competition-public-bracket-match { border-color:color-mix(in srgb,var(--success) 45%,var(--border)); }
</style>
