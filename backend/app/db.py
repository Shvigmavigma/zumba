...
async def init_db() -> None:
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS exclude_from_rer BOOLEAN DEFAULT FALSE"))
        await conn.execute(text("UPDATE users SET exclude_from_rer = FALSE WHERE exclude_from_rer IS NULL"))
        await conn.execute(text("UPDATE users SET exclude_from_rer = TRUE WHERE login = :admin_login"), {"admin_login": settings.admin_login})
        await conn.execute(text("ALTER TABLE users ALTER COLUMN exclude_from_rer SET DEFAULT FALSE"))
        await conn.execute(text("ALTER TABLE users ALTER COLUMN exclude_from_rer SET NOT NULL"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS rating_adjustments JSONB DEFAULT '{}'::jsonb"))
        ...
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS device_fingerprint VARCHAR(64)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS device_label VARCHAR(120)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS ip_fingerprint VARCHAR(64)"))
        await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_users_device_fingerprint ON users (device_fingerprint)"))
        await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_users_ip_fingerprint ON users (ip_fingerprint)"))
        ...
        await conn.execute(text("ALTER TABLE races ADD COLUMN IF NOT EXISTS registration_start TIMESTAMP WITH TIME ZONE"))
        await conn.execute(text("ALTER TABLE races ADD COLUMN IF NOT EXISTS practice_start_time TIME WITHOUT TIME ZONE"))
        await conn.execute(text("ALTER TABLE races ADD COLUMN IF NOT EXISTS practice_end_time TIME WITHOUT TIME ZONE"))
        await conn.execute(text("ALTER TABLE races ADD COLUMN IF NOT EXISTS qualification_start_time TIME WITHOUT TIME ZONE"))
        await conn.execute(text("ALTER TABLE races ADD COLUMN IF NOT EXISTS qualification_end_time TIME WITHOUT TIME ZONE"))
        await conn.execute(text("ALTER TABLE races ADD COLUMN IF NOT EXISTS race_session_start_time TIME WITHOUT TIME ZONE"))
        await conn.execute(text("ALTER TABLE races ADD COLUMN IF NOT EXISTS race_session_end_time TIME WITHOUT TIME ZONE"))
        await conn.execute(text("UPDATE races SET registration_start = COALESCE(registration_start, created_at, datetime_start) WHERE registration_start IS NULL"))
        await conn.execute(text("ALTER TABLE races ALTER COLUMN registration_start SET DEFAULT CURRENT_TIMESTAMP"))
        await conn.execute(text("ALTER TABLE races ALTER COLUMN registration_start SET NOT NULL"))
        await conn.execute(text("ALTER TABLE races ADD COLUMN IF NOT EXISTS weather_chances JSONB DEFAULT jsonb_build_object('clear', 100, 'partly_cloudy', 0, 'overcast', 0, 'light_rain', 0, 'heavy_rain', 0, 'storm', 0)"))
        await conn.execute(text("ALTER TABLE races ADD COLUMN IF NOT EXISTS track_temperature NUMERIC(5, 1)"))
        await conn.execute(text("UPDATE races SET weather_chances = jsonb_build_object('clear', 100, 'partly_cloudy', 0, 'overcast', 0, 'light_rain', 0, 'heavy_rain', 0, 'storm', 0) WHERE weather_chances IS NULL OR jsonb_typeof(weather_chances) <> 'object'"))
        await conn.execute(text("ALTER TABLE races ALTER COLUMN weather_chances SET DEFAULT jsonb_build_object('clear', 100, 'partly_cloudy', 0, 'overcast', 0, 'light_rain', 0, 'heavy_rain', 0, 'storm', 0)"))
        await conn.execute(text("ALTER TABLE races ALTER COLUMN weather_chances SET NOT NULL"))
        ...
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS favorite_car VARCHAR(80)"))
